...
//DADOS GLOBAIS
if (isset($_GET['data']))
    $todasAsFolhas = $objConexaoRelatorio->Consultar("SELECT folha_de_rosto.cod, folha_de_rosto.lancamento_almoxarife as 'datas', count(folha_de_rosto.lancamento_almoxarife) as 'contagem' FROM `folha_de_rosto` 
    where folha_de_rosto.lancamento_almoxarife like '%".$_GET['data']."%' 
    GROUP BY folha_de_rosto.lancamento_almoxarife  
    ORDER BY `cod` ASC");
    
else
    $todasAsFolhas = $objConexaoRelatorio->Consultar("SELECT folha_de_rosto.cod, folha_de_rosto.lancamento_almoxarife as 'datas', count(folha_de_rosto.lancamento_almoxarife) as 'contagem' FROM `folha_de_rosto` 
    where folha_de_rosto.lancamento_almoxarife like '%%' 
    GROUP BY folha_de_rosto.lancamento_almoxarife  
    ORDER BY `cod` ASC");
